...
// Using any type to avoid complex type checking for now
interface MediaItem {
  embyId: string;
  name: string;
  type: string;
    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
        {items.map((item) => (
          <div 
            key={item.embyId}
            className="group cursor-pointer bg-transparent border-none hover:bg-gray-800/20 transition-colors rounded-xl"
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
          ...
          >
            ...
            <div className="mt-3">
              ...
              <p className="text-gray-400 text-xs mt-1">
                {formatDuration(item.runtime || undefined)}
              </p>
            </div>
          </div>
        ))}
