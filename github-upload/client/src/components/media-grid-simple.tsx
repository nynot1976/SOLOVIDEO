    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
        {items.map((item) => (
          <button 
            key={item.embyId}
            className="group cursor-pointer hover:bg-gray-800/20 transition-colors rounded-xl p-2 border-none bg-transparent w-full text-left"
            onPointerDown={(e) => {
              e.preventDefault();
              e.stopPropagation();
            ...
            onTouchEnd={(e) => {
              e.preventDefault();
              e.stopPropagation();
              console.log('Touch end - Card clicked:', item.name);
              console.log('Media clicked:', item.name, 'Type:', item.type);
              console.log('Opening media details for:', item.name);
              onItemClick(item);
            }}
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              console.log('Click - Card clicked:', item.name);
              console.log('Media clicked:', item.name, 'Type:', item.type);
              console.log('Opening media details for:', item.name);
              onItemClick(item);
            }}
